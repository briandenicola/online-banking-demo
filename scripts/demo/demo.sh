...
register_identity() {
  ...
  case "$HTTP_STATUS" in
    ...
    400|409)
      if grep -qi 'already' <<<"$HTTP_BODY"; then
...
seed_accounts() {
  ...
  while read -r owner; do
    ...
    while [[ $n -lt $want_total ]]; do
      local spec have
      spec=$(jq --arg o "$owner" --argjson i "$n" '[.accounts[] | select(.owner == $o)][$i]' <<<"$DATASET")
      local acct_type
      acct_type=$(jget_from "$spec" '.accountType')

      # Idempotence by (owner, type, ordinal): how many of this type already exist?
      have=$(jq --arg t "$acct_type" '[.[] | select((.accountType // .AccountType) == $t)] | length' <<<"$existing")
      local want_of_type
      want_of_type=$(jq --arg o "$owner" --arg t "$acct_type" \
        '[.accounts[] | select(.owner == $o and .accountType == $t)] | length' <<<"$DATASET")

      if [[ "$have" -ge "$want_of_type" ]]; then
        detail "${owner}: ${acct_type} already present (${have})"
      else
        local body
...
seed_transactions() {
  ...
  while read -r owner; do
    ...
    [[ ${#ids[@]} -gt 0 ]] || die "'${owner}' has no accounts; cannot post transactions."
    ...
    for ((idx = 0; idx < count; idx++)); do
      ...
      account_id="${ids[$account_index]:-${ids[0]}}"

      http_or_die GET "/api/transactions/account/${account_id}" "" "$token" \
        "Could not list transactions for account ${account_id}." 200
      if jq -e --arg d "$description" 'any(.[]; (.description // .Description) == $d)' >/dev/null <<<"$HTTP_BODY"; then
...
propose() {
  ...
  body=$(jq -n \
    ...
    --argjson p "$payload" --argjson e "$evidence" --argjson f "$facts" \
    --arg sup "$supersedes" \
...
seed_approvals() {
  ...
  for ((idx = 0; idx < count; idx++)); do
    ...
    fired=$(jq -r '[.firedEscalators[].key] | join(", ")' <<<"$HTTP_BODY")
  ...
  while read -r owner; do
    ...
    if [[ "$HTTP_STATUS" == "200" ]]; then
      ...
      while read -r aid; do
        ...
        [[ "$HTTP_STATUS" == "200" ]] && n_tx=$((n_tx + $(jq 'length' <<<"$HTTP_BODY")))
