...
async def handle_chat(request: ChatRequest, http_request: Request, user: UserContext) -> ChatResponse:
    """Get financial advice from the AI agent."""
    if not agent_service.agent_ready or not agent_service.financial_agent:
        raise HTTPException(
            status_code=503,
    ...
    try:
        with tracer.start_as_current_span("ai-agent.chat") as span:
            ...
            span.set_attribute("agent.model", agent_service.model_name)
            ...
            if authenticated_user_id not in agent_service.user_sessions:
                agent_service.user_sessions[authenticated_user_id] = agent_service.financial_agent.create_session()

            session = agent_service.user_sessions[authenticated_user_id]

            result = await agent_service.financial_agent.run(user_content, session=session)
            answer = str(result) if result else "I couldn't generate a response at this time."

            # Persist both messages to Cosmos
            await agent_service.save_chat_message(authenticated_user_id, "user", request.message)
            await agent_service.save_chat_message(authenticated_user_id, "assistant", answer)
...
async def start_new_session(request: ChatRequest, http_request: Request, user: UserContext) -> ChatResponse:
    """Start a new chat session (clears conversation history for this user)."""
    agent_service.user_sessions.pop(user.user_id, None)
    return await handle_chat(request, http_request, user)


async def get_chat_history(user_id: str, user: UserContext) -> dict:
    ...
    messages = await agent_service.load_chat_history(user_id)
...
def get_health_status() -> dict:
    return {
        "status": "healthy",
        "agent_ready": agent_service.agent_ready,
...
async def foundry_status() -> dict:
    ...
    try:
        session = agent_service.financial_agent.create_session()
        response = await agent_service.financial_agent.run("ping", session=session)
