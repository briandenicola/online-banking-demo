...
resource "azapi_resource" "storage_outbound_rule" {
  ...
  depends_on = [
    time_sleep.wait_storage,  # 10m wait for backing service + PE
...
1. **10 minutes per backing service**: Wait after each backing service (Storage, Cosmos, AI Search) and its private endpoint are created
...
**Total clean provisioning time: 30+ minutes**
...
# List outbound rules (explicitly created)
az rest --method get \
