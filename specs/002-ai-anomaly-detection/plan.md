...
Replace the ai-service's local IsolationForest ML model with Azure AI Foundry (GPT-5.4-mini) for risk scoring. Add an "All Transactions" admin view showing every transaction with its AI-generated risk score.
...
- **Current ai-service**: Python/FastAPI, consumes Redis Stream `banking-events`, stores flagged txns in Redis
- **Admin UI**: React/MUI, fetches from `/api/admin/stats` and `/api/admin/flagged-transactions`
- **Istio routing**: `/api/admin` → ai-service (already configured)
...
### T5: Update ai-service requirements and Dockerfile
**Files**: `src/ai-service/requirements.txt`, `src/ai-service/Dockerfile`
...
### T6: Update K8s deployment for ai-service
**File**: `deploy/kustomize/base/ai-service.yaml`
...
### T10: Update ai-service tests
**File**: `src/ai-service/tests/`
...
- Build ai-service container to ACR
